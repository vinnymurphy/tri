...
os.chdir(proj_path)  # This is so my local_settings.py gets loaded.
...
def weekly_urls():
    ...
    weeks = []
    weeks.extend(sorted([u for u in urls if re.search(r'week\d+.htm', u)]))
    weeks.extend(sorted([u for u in urls if re.search(r'weekp\d+.htm', u)]))
    weeks.extend(sorted([u for u in urls if re.search(r'weekc\d+.htm', u)]))
    weeks.extend(sorted([u for u in urls if re.search(r'taper\d+.htm', u)]))
    return weeks
...
def get_minutes(time_string):
    ...
    h, m = list(map(int, time_string.split(':')))
    return h * 60 + m
...
def workouts(txt):
    ...
    dd = defaultdict(dict)
    sbr_re = re.compile(r'(%s)\s+(\d+:\d+)' % r'|'.join(workout_type))
    sport = None
